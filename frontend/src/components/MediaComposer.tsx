...
export function MediaComposer({
  ...
  const [pickerAccept, setPickerAccept] = useState("image/*,audio/*,video/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.txt,.csv");
  ...
  const isSendDisabled = disabled || (!inputMessage.trim() && !pendingAttachment) || isUploadingMedia;
  const fileSizeLabel = pendingAttachment ? `${(pendingAttachment.size / (1024 * 1024)).toFixed(2)} MB` : "";

  return (
    <div className="flex flex-col gap-2">
      {pendingAttachment ? (
        <div className={`flex flex-col gap-2 rounded-xl border p-3 text-sm ${darkMode ? "border-slate-700 bg-slate-800/80 text-slate-200" : "border-slate-200 bg-slate-50 text-slate-700"}`}>
          <div className="flex items-center justify-between gap-2">
            ...
            <button type="button" onClick={onRemoveAttachment} className="rounded-full p-1 hover:bg-white/10" aria-label="Quitar adjunto">
              <X size={16} />
            </button>
          </div>
          ...
          {pendingAttachment.type?.startsWith("image/") ? (
            <div className="overflow-hidden rounded-lg border border-slate-700/60 bg-slate-900/50 p-2">
              <img src={previewUrl || undefined} alt="Preview del adjunto" className="max-h-48 w-full rounded-lg object-contain" />
            </div>
            ...
            <div className="overflow-hidden rounded-lg border border-slate-700/60 bg-slate-900/50 p-2">
              <video controls src={previewUrl || undefined} className="w-full max-h-48 rounded-lg" />
            </div>
          ) : (
            <div className={`rounded-xl border px-3 py-3 text-sm ${darkMode ? "border-slate-700 bg-slate-900/70 text-slate-300" : "border-slate-200 bg-white text-slate-600"}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {previewUrl ? (
                    <a
                      ...
                      className={`mt-2 inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[11px] font-medium ${darkMode ? "bg-slate-800 text-slate-100 hover:bg-slate-700" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
                    >
                      <Download size={12} />
                      Descargar
                    </a>
      ...
      <div className="relative flex gap-2">
        ...
        <div className="relative self-end">
          ...
          {isMenuOpen ? (
            <div className="absolute bottom-14 left-0 z-20 min-w-[170px] rounded-xl border border-slate-700 bg-slate-900/95 p-2 shadow-xl backdrop-blur">
              <button type="button" onClick={() => openPicker("image/*")} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-slate-200 hover:bg-slate-800">
                <ImageIcon size={15} />
                Imagen
              </button>
              <button type="button" onClick={() => openPicker("video/*")} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-slate-200 hover:bg-slate-800">
                <VideoIcon size={15} />
                Video
              </button>
              <button type="button" onClick={() => openPicker("audio/*")} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-slate-200 hover:bg-slate-800">
                <Music4 size={15} />
                Audio
              </button>
              <button type="button" onClick={() => openPicker(".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.txt,.csv")} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-slate-200 hover:bg-slate-800">
                <FileText size={15} />
                Documento
              </button>
            </div>
        ...
        <textarea
          ...
          onKeyDown={onKeyDown}
          placeholder={disabled ? "Selecciona una conversación para responder" : "Responder como asesor..."}
          disabled={disabled}
          rows={1}
